...
When creating a new *payment contact*:

- MUST receive user authorization to reveal payment information to the contact.
- If the node has previously received a payment from this contact including the
  `invreq_contact_secret` field:
  - MUST associate the received `invreq_contact_secret`, `invreq_payer_offer`
    and `invreq_payer_bip_353_name` with this contact.
  - If `invreq_payer_bip_353_name` was included:
    - SHOULD use it to fetch an `invreq_payer_offer` if none was included.
    - MUST verify that the received offer matches the `offer_signing_key`
      provided in `invreq_payer_bip_353_signature`.
    - SHOULD use it to refresh the `invreq_payer_offer` if it expires.
    - MAY use it to refresh the `invreq_payer_offer` periodically.
- Otherwise:
  - MUST generate a unique `invreq_contact_secret` for that contact, using
...
- If they want the recipient to be able to identify who paid them:
  - If the recipient is not a *payment contact*:
    - MUST seek user authorization to create a *payment contact* matching that
      recipient.
...
- If `invreq_payer_bip_353_name` is provided:
...
- If `invreq_contact_secret` was included in the `invoice_request`:
  - If it matches an existing *payment contact*:
    - SHOULD display the `invreq_payer_note`, if one is provided.
    - MUST ignore `invreq_payer_offer` and `invreq_payer_bip_353_name`.
  - Otherwise:
    - MAY seek authorization from the user to add a new contact or update an
      existing contact based on this received payment (if the user verified
      that the payment came from this contact).
